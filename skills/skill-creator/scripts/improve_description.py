...
by calling opencode CLI as a subprocess (same auth pattern as run_eval.py —
...
def _call_opencode(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run opencode CLI with the prompt on stdin and return the text response.
    ...
    # Remove OPENCODE env var to allow nesting opencode CLI inside a
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"opencode CLI exited {result.returncode}\nstderr: {result.stderr}"
        )
